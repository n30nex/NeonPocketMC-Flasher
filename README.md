...
DeskOS **1.8.0-rc.2** is the current release candidate. It improves radio timing and diagnostics, loading progress, and map labels. The browser reads real DeskOS JSON, waits for startup over one USB connection, and verifies the display, identity, radio, and nested SD status. Preserving updates can finish without optional SD storage; fresh installs retain the complete setup. Hardware/build choices stay locked while flashing, and verification is invalidated when the selection changes.
